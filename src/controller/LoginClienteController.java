	...
	protected void doProcessa(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (autenticar(email, senha) == true) {
				...
				sessao.removeAttribute("error");
				sessao.setAttribute("email", email);
				sessao.setAttribute("funcionario", ClienteDAO.FuncionarioEx(email));
				request.getRequestDispatcher("/index.jsp").forward(request, response);
			} else {
				HttpSession sessao = request.getSession();
				sessao.setAttribute("error", "Usuario não encontrado!");
				request.getRequestDispatcher("/shop-ui-login.jsp").forward(request, response);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
